...
class _HomeViewState extends State<HomeView> {
            ...
            Center(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Vspacer(height: 15),
                  _workoutTypeList(user.workoutPrograms != null
        ...
        children: [
          ...monitoring.map((bodyMonitor) {
            return SizedBox(
              // height: 180,
              width: 170,
              child: bodyMonitorCell(
                  bodyMonitor, monitoring.indexOf(bodyMonitor), colors),
            );
          }),
        ],
      ),
        ...
        children: [
          ...types.map((type) {
            return Container(
              ...
              height: 60,
              child:
                  workoutProgramTypeCell(type, type == _currentPrograms?.type),
            );
          }),
        ],
      ),
  ...
  Widget workoutProgramTypeCell(String type, bool isSelected) {
    return OutlinedButton(
      onPressed: () {
        _currentPrograms = _user?.workoutPrograms
      ...
      style: OutlinedButton.styleFrom(
        ...
        side: isSelected
            ? BorderSide.none
            : const BorderSide(color: Color(0xfffaf1e6)),
  ...
  Widget _userImage(String imageName) {
    return roundedImage(
        imageUrl: imageName, radius: 25, padding: 2, isNetworkImage: true);
  }
...
Widget bodyMonitorCell(BodyMonitors bodyMonitors, int index, List<Color> colors,
    {double? width, double leftPadding = 15}) {
  return Padding(
